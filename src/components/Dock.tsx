...
export function Dock({ isMobile, onOpenApp }: { isMobile: boolean, onOpenApp?: (app: string) => void }) {
    ...
    const handleTrashClick = () => {
        const msg = TRASH_RESPONSES[Math.floor(Math.random() * TRASH_RESPONSES.length)];
        setTrashToast(msg);
            ...
            <AnimatePresence>
                {trashToast && (
                    <motion.div
                        ...
                        className="fixed bottom-[100px] left-1/2 z-[9999] px-5 py-3 rounded-[16px] text-[13px] text-white font-medium max-w-[90vw] text-center"
                        style={{
